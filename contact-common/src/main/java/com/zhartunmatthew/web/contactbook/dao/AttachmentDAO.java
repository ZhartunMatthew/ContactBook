...
public class AttachmentDAO extends AbstractDAO<Long, Attachment> {
    ...
    private static final String INSERT_ATTACHMENT_QUERY =
        ...
        "(contact_id, file_name, upload_date, comment) VALUES (?, ?, ?, ?);";
    ...
    public ArrayList<Attachment> readByContactId(Long contactId)
            throws DAOException {
        ArrayList<Attachment> attachments = new ArrayList<>();
        ResultSet attachmentResultSet;
        try (PreparedStatement statement =
                     connection.prepareStatement(SELECT_CONTACT_ATTACHMENTS)) {
            ...
            throw new DAOException(
                    String.format("Can't read Attachments by contact id = %s",
                            contactId), ex);
        }
        return attachments;
    }
    ...
    public void insert(Attachment val) throws DAOException {
        try (PreparedStatement statement =
                     connection.prepareStatement(INSERT_ATTACHMENT_QUERY,
    ...
    public Attachment read(Long id) throws DAOException {
        ...
        try (PreparedStatement statement =
                     connection.prepareStatement(SELECT_ATTACHMENT_BY_ID)) {
            statement.setLong(1, id);
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                attachment = (Attachment)
                        EntityFactory.createEntityFromResultSet(
                                resultSet, Attachment.class);
            }
        } catch (SQLException ex) {
            throw new DAOException(
                    String.format("Can't read Attachment with id = %d", id), ex);
        }
        return attachment;
    }
    ...
    public void update(Long id, Attachment val) throws DAOException {
        try (PreparedStatement statement =
                     connection.prepareStatement(UPDATE_ATTACHMENT)) {
            ...
            throw new DAOException(
                    String.format("Can't update Attachment with id = %d", id), ex);
        }
    }
    ...
    public void delete(Long id) throws DAOException {
        try (PreparedStatement statement =
                     connection.prepareStatement(DELETE_ATTACHMENT)) {
            statement.setLong(1, id);
            statement.execute();
        } catch (SQLException ex) {
            throw new DAOException(
                    String.format("Can't delete Attachment with id = %d", id), ex);
        }
    }

    public void deleteByContactId(Long id) throws DAOException {
        try (PreparedStatement statement =
                     connection.prepareStatement(DELETE_CONTACT_ATTACHMENTS)) {
            statement.setLong(1, id);
            statement.execute();
        } catch (SQLException ex) {
            throw new DAOException(
                    String.format("Can't delete Attachments with contact id = %d", id), ex);
        }
    }
}
